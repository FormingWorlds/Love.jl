...
G = 6.6743e-11
e = 0.0041
# ρₛ = Float64[2422, 2422] 
# r = Float64[0, 0.1, 191.1] .* 1e3
# μ = Float64[1, 1] .* 1e9
# κ = Float64[10e9, 10e9]
# η = Float64[1.0, 1.0] 
...
Ic = get_Ic(r[end,1], ρₛ[1], g[end,1], μ[1], "solid", 6, 3)

ηs = 10 .^ collect(10:0.5:20)
...
# ρₛ = Float64[2422, 2422, 2422, 2422] 
# r = Float64[0, 0.1, 100.0, 150.0, 191.1] .* 1e3
# μ = Float64[1, 1, 1, 1] .* 1e9
# κ = Float64[10e9, 10e9, 10e9, 10e9]
# η = Float64[1.0, 1.0, 1.0, 1.0] 
...
for i in eachindex(ηs) 
    ...
    μc = copy(μ)
    # μc[:]
    # μc[3] = 1im*ω*μ[3] ./ (1im*ω .+ μ[3] ./ (η[3] .* ηs[i]))
...
ax2.loglog(ηs, Edot/1e9, color="C1", label="Single Layer")
ax2.loglog(ηs, Edot2/1e9, "k:", label="Multilayer")
