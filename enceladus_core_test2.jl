...
for i in eachindex(μs) 
    ...
    Pl = zeros(Float64, 8,4)
    Pl[7,4] = 1

    Ic = get_Ic(r[end,1], ρₛ[1], g[end,1], μ[1], "liquid", 8, 4)
    yR = Bprod1[:,:,end,end]*(Ic + Pl)

    # println("Here is another B*Ic")
    # display(Bprod1[:,:,end,end]*(Ic + Pl))


    y2 = calculate_y(r, ρ, g, μc, κ, ω, ρₗ, κₗ, ηₗ, ϕ, k)
